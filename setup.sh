...
SSH_ID=`doctl compute ssh-key list | grep "Ted" | cut -d' ' -f1`
...
# Copy orig scripts to runnable location
cp master.sh.orig master.sh
cp node.sh.orig node.sh
...
# Create Master
doctl compute droplet create k8s-master \
	--region $REGION \
...
MASTER_ID=`doctl compute droplet list | grep "k8s-master" |cut -d' ' -f1`
...
# Join Nodes
doctl compute droplet create k8s-node1 k8s-node2 k8s-node3 \
	--region $REGION \
	--image ubuntu-16-04-x64 \
	--size 1gb \
	--tag-name k8s-node \
